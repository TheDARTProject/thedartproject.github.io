...
function animatePings(timestamp) {
    ...
    // Check if any pings should be activated
    pings.forEach(ping => {
        if (!ping.active && now >= ping.nextPingTime && activePings.length < 50) {
            createPing(ping);
            ping.nextPingTime = now + Math.random() * 60000 + 30000; // Set next ping time 30-90 seconds later
        }
    });
