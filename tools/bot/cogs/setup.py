...
class SetupCog(commands.Cog):
    ...
    @app_commands.command(name="setup", description="Set up the bot to monitor specific channels.")
    async def setup(self, interaction: discord.Interaction):
        logger = configure_server_logging(interaction.guild.id)
        logger.info(f"Setup command invoked by {interaction.user.name} (Server: {interaction.guild.name})")
        if interaction.guild is None:
            await interaction.response.send_message("Please set up the bot in a server, not in DMs.", ephemeral=True)
            return
        ...
        async def select_callback(select_interaction):
            ...
            async def yes_callback(button_interaction):
                for channel in selected_channels:
                    ...
                    await button_interaction.response.edit_message(embed=update_embed, view=None)
                ...
                self.save_server_config(interaction.guild, selected_channels)
            ...
            async def no_callback(button_interaction):
                ...
                await button_interaction.response.edit_message(embed=finish_embed, view=None)

                # Save the server settings
                self.save_server_config(interaction.guild, selected_channels)
                logger.info(f"Monitoring started for guild: {interaction.guild.name} (without historical scan)")
            ...
            await select_interaction.response.edit_message(embed=step2_embed, view=step2_view)
    ...
    async def historical_scan(self, channel):
        ...
        logger.info(f"Starting historical scan for channel: {channel.name} (Server: {channel.guild.name})")
        ...
        logger.info(f"Historical scan completed for channel: {channel.name} (Server: {channel.guild.name})")

    def save_server_config(self, guild, selected_channels):
