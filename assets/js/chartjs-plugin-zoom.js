...
(function(global, factory) {
  ...
  function getEnabledScalesByPoint(options, point, chart) {
    const {
      mode = 'xy', scaleMode, overScaleMode
    } = options || {};
  ...
  function zoom(chart, amount, transition = 'none') {
    const {
      x = 1, y = 1, focalPoint = getCenter(chart)
    ...
    helpers.callback(zoomOptions.onZoom, [{
      chart
    }]);
  }
  ...
  function resetZoom(chart, transition = 'default') {
    ...
    chart.update(transition);
    helpers.callback(state.options.zoom.onZoomComplete, [{
      chart
    }]);
  }
  ...
  function zoomStart(chart, event, zoomOptions) {
    ...
    if (onZoomStart) {
      ...
      if (helpers.callback(onZoomStart, [{
        chart,
        event,
        point
      }]) === false) {
        helpers.callback(onZoomRejected, [{
          chart,
          event
        }]);
        return false;
  ...
  function mouseDown(chart, event) {
    ...
    if (
      ...
      return helpers.callback(zoomOptions.onZoomRejected, [{
        chart,
        event
      }]);
    }
  ...
  function mouseUp(chart, event) {
    ...
    helpers.callback(onZoomComplete, [{
      chart
    }]);
  }
  ...
  function wheelPreconditions(chart, event, zoomOptions) {
    ...
    if (keyNotPressed(getModifierKey(zoomOptions.wheel), event)) {
      helpers.callback(zoomOptions.onZoomRejected, [{
        chart,
        event
      }]);
      return;
  ...
  function addDebouncedHandler(chart, name, handler, delay) {
    if (handler) {
      getState(chart).handlers[name] = debounce(() => helpers.callback(handler, [{
        chart
      }]), delay);
    }
  }
  ...
  function createEnabler(chart, state) {
    return function(recognizer, event) {
      ...
      if (!state.panning && event.pointerType === 'mouse' && (
        ...
        helpers.callback(panOptions.onPanRejected, [{
          chart,
          event
        }]);
        return false;
  ...
  function startPan(chart, state, event) {
    ...
    if (helpers.callback(onPanStart, [{
      chart,
      event,
      point
    }]) === false) {
      return helpers.callback(onPanRejected, [{
        chart,
        event
      }]);
    }
  ...
  function endPan(chart, state) {
    ...
    if (state.panning) {
      ...
      helpers.callback(state.options.pan.onPanComplete, [{
        chart
      }]);
    }
  }
